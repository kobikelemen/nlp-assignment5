...
if args.function == 'pretrain':
    ...
    # pretrain_text = open(args.reading_params_path, 'r').read()
    train_dataset = NameDataset(pretrain_dataset, text) # one line of poem is roughly 50 characters
    tconf = TrainerConfig(max_epochs=75, batch_size=256, learning_rate=args.finetune_lr,
                      lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                      ckpt_path=args.writing_params_path, num_workers=4, writer=writer)

    trainer = Trainer(gptmodel, train_dataset, None, tconf)
    trainer.train()
